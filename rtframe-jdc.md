...
## 组件运行时，重构时，都需要向每个运行组件的core，下发邻居部署信息。部署信息下发到各个核，局部重构后，下发部署变化信息到各个核，下发部署信息到重构后的核。
...
=======
>>>>>>> 8fbe1cf3f05adfe857ea23f0d9a74faa8a01a508
...
## 增加节点管理类型代理，节点管理上报的状态在前端需要显示，节点管理芯片有对应的控制页面
...
##  芯片列表增加芯片id列表，并增加型号及芯片类型栏。
原始需求：   这个是高逸龙提出的，在芯片列表没有芯片唯一标识芯片id，使用上有不方便。

## 2022年2月完成 rtframe 高可用